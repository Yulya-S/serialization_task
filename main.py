...
class Widget():
    ...
    def from_binary(self, data, parent=""):
        ...
        len_value = 4 + len_name + struct.unpack("i", data[4 + len_name:4 + len_name + 4])[0]
        if name == "ComboBox":
            value = self.types_from_binary_for_list(self, data[4 + len_name:len_value + 4])
        elif name == "MainWindow":
            value = self.types_from_binary_for_list(self, data[4 + len_name:len_value + 4], "s")
        else:
            value = self.types_from_binary_for_list(self, data[4 + len_name:len_value + 4], "i")
